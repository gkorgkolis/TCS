...
from cd_methods.lcm_mini.src.modules.informer_module import InformerModule
from cd_methods.lcm_mini.src.utils.utils import lagged_batch_crosscorrelation
...
def run_inv_pcmci(
    ...
    flip (bool) : (optional) if true, it flips the adjacency matrix in order to match the lag order of CP (larger lags first);
...
def estimate_with_PCMCI(true_data: pd.DataFrame, n_lags: int=None, n_reps: int=None) -> tuple:
    ...
    #     n_reps -= 1
    if adj_cp.sum()==0: 
        raise ValueError("ValueError: PCMCI was not able to find a valuable solution - no edges were discovered.")
    else:
...
def estimate_with_LCM(
    ...
    data_pd.rename(columns=dict(zip(data_pd.columns, list(string.ascii_uppercase)[:len(list(data_pd.columns))])), inplace=True)
    ...
    adj_cp = torch.Tensor(_from_full_to_cp(adj_pd))
...
def estimate_with_CP(
    ...
    if model is None:
        model = Architecture_PL.load_from_checkpoint("../cd_methods/CausalPretraining/res/lcm_CI_CR_RH_12_3_merged_340k.ckpt")
    else:
