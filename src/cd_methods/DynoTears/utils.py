...
import numpy as np
import pandas as pd
from utils import (_from_full_to_cp, group_lagged_nodes, regular_order_pd)
...
def rename_sm_nodes(pred_pd):
    ...
    Return
    ------
    out (pandas.DataFrame) : the Pandas adjacency matrix, renamed
    """
...
def run_dynotears(
    ...
    Args
    ----
    data (pandas.DataFrame) : the input time-series sample
    ...
    Return
    ------
    out (pandas.DataFrame) : the full time adjacency matrix as a pd.DataFrame 
    """
...
def estimate_with_DYNOTEARS(
    ...
    n_lags (int) : the maximum number of lags; (default = 1)
    lambda_w (float) : the lambda_w internal parameter of DYNOTEARS; (default = 0.1)
    lambda_a (float) : the lambda_a internal parameter of DYNOTEARS; (default = 0.1)
    max_iter (int) : the max_iter optimization parameters of DYNOTEARS; (default = 100)
    n_reps (int) : the maximum number of repetitions; used when no edges are found; (default = 10)
