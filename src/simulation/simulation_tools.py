...
from simulation.memoization import memoclean
...
# # NOTE: Legacy version of the search configurations. Necessary for the DENSE experiment.
# from simulation.simulation_configs import cd_config as CD_CONFIGS
# from simulation.simulation_configs import noise_config as NZ_CONFIGS
# from simulation.simulation_configs import pred_config as FC_CONFIGS
...
def get_optimal_sim_XY(
    ...
    for cfg in CONFIGS:
        ...
        # NOTE: latest version
        res = run_detection_metrics(real=true_data, synthetic=simulated_data, bias_correction=bias_correction, verbose=False)
        ...
        # auc, probs, labels = optimal_det_func(true_data, simulated_data, **optimal_det_config)
        ...
        # print(f" - LOG: DEBUGGING: Probs={len(probs)} | labels={len(labels)} (Y len: {labels[:]})")

        if verbose:
            print(f"LOG: Optimal Simulation: Max C2ST AUC achieved: {auc} ({optimal_det_func.__name__})")
    ...
    # # NOTE: trim test predictions & probabilities to avoid inconsistencies (probably not needed anymore after recent changes - check)
    # min_pred_len = min([len(x) for x in probs_list])
    # probs_list = [x[-min_pred_len:] for x in probs_list]
    # labels_list = [x[-min_pred_len:] for x in labels_list]
...
# @memoclean
def get_optimal_sim_XYP(
        true_data: pd.DataFrame, 
        CONFIGS: dict = None, 
        init: bool = False, 
        sparsity_penalty: bool = True,
    ...
    true_data (pandas.DataFrame) : the real data
    CONFIGS (dict) : Dictionary containing the search spaces for the optimal simulation configuration (default = `None`)
    init (bool) : If True, initializes the scm with the first `max lag` samples of the real data (default = `False`) 
    sparsity_penalty (bool) : If `True`, it performs a permutation-based statistical test on the discriminator's predicted 
                    probabilities (details in `TCS ref` ), to find configurations with AUC statistical equivalent to the 
                    current optimal; the returned configuration is the sparsest one between those that are found equivalent. 
                    If no such configuration exists, the returned configuration is the one with the optimal detection auc, 
                    no matter the structure density (default = `True`)
    archive_path (str) : The path to the archive file where results of all configurations will be saved (default = `None`); 
                    if `None`, the archive will not be saved
    verbose (bool) : Whether or not to print descriptive log messages during the process (default = `True`)
    ...
    def parallel_simulation(
        ...
        try:  
            ...
            res = run_detection_metrics(real=true_data, synthetic=simulated_data)
            optimal_det_config = res["config"]
            optimal_det_func = res["detector"]
            labels = res["labels"]
            probs = res["probs"]
            ...
            return {
                ...
                "scores" : scores,
                "fit_scm" : fit_scm,
                "optimal_det_func" : optimal_det_func,
                "optimal_det_config" : optimal_det_config,
    ...
    res_pooled = pool.starmap(parallel_simulation, [[cfg, true_data] for cfg in CONFIGS])
    pool.close()
    pool.join()
    
    simulated_data_list = [item["simulated_data"] for item in res_pooled if item["simulated_data"] is not None]
    det_config_list = [item["optimal_det_config"] for item in res_pooled if item["optimal_det_config"] is not None]
    det_func_list = [item["optimal_det_func"] for item in res_pooled if item["optimal_det_func"] is not None]
    fit_scm_list = [item["fit_scm"] for item in res_pooled if item["fit_scm"] is not None]
    scores_list = [item["scores"] for item in res_pooled if item["scores"] is not None]
    labels_list = [item["labels"] for item in res_pooled if item["labels"] is not None]
    probs_list = [item["probs"] for item in res_pooled if item["probs"] is not None]
    auc_list = [item["auc"] for item in res_pooled if item["auc"] is not None]
    cfg_list = [item["cfg"] for item in res_pooled if item["cfg"] is not None]
    ...
    if archive_path is not None:
        with open(archive_path, "wb") as f:
            dill.dump(archive, f)
        if verbose:
            print(f"LOG: Optimal Simulation: archive saved at {archive_path} ...")
...
def archive_sparsity_penalty(archive: dict, a: float = 0.05, n: int = 100):
    """ """
    simulated_data_list = archive["simulated_data_list"]
    fit_scm_list = archive["fit_scm_list"]
    scores_list = archive["scores_list"]
    labels_list = archive["labels_list"]
    probs_list = archive["probs_list"]
    auc_list = archive["auc_list"]
    cfg_list = archive["cfg_list"]
    
    # derive the # edges per configuration
    cp_list = [x.causal_structure.causal_structure_cp.sum().item() for x in fit_scm_list]
...
def get_random_sim_XY(
    ...
    """ 
    Runs the simulation process for one random combination. Specifically used only as a baseline during validation.
    ...
    if verbose:
        print(f"LOG: Choosing a random configuration ...")
    ...
    rand = np.random.randint(len(CONFIGS))
    cfg = CONFIGS[rand]
    ...
    # NOTE: latest version
    res = run_detection_metrics(real=true_data, synthetic=simulated_data, bias_correction=False, verbose=False)
    optimal_det_config = res["config"]
    optimal_det_func = res["detector"]
    labels = res["labels"]
    probs = res["probs"]
    ...
    if verbose:
        print(f"LOG: Optimal Simulation ({auc_list[0]} auc): chosen configuration: {json.dumps(CONFIGS[rand], sort_keys=True, indent=4)}")

    res = {
        "optimal_data" : simulated_data_list[0],
        "optimal_scm" : fit_scm_list[0],
        "optimal_config": CONFIGS[0], 
        "optimal_detector" : det_func_list[0], 
        "optimal_detector_config": det_config_list[0], 
...
# # # NOTE: Older versions (kept as legacy - consider to remove later on)
# def get_optimal_sim(
#         true_data: pd.DataFrame, 
#         CONFIGS: dict = None, 
#         done_eval: bool = False,
