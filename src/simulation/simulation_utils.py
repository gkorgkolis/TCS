...
from utils import (_from_cp_to_full, _from_full_to_cp, estimate_with_CP, estimate_with_LCM,
                   estimate_with_PCMCI, group_lagged_nodes, r2_from_scratch,
                   regular_order_pd)
from simulation.memoization import DiskMemoize, DiskMemoize

# absolute path to the cd_configs.yaml file
with open('c:\\Users\\skypl\\Documents\\GitHub\\TCS\\src\\configs\\simulation\\cd_configs.yaml','r') as f:
...
def simulate(
    ...
    if init:
        fit_scm._reset_time_series(init_history=true_data.loc[:n_lags, :])
...
def safe_cd_task(
    ...
    cd_method (str) : The specified CD method. Default is `PCMCI`.
    cd_kwargs (dict) : The configuration of the specified CD method. Default is `{"n_lags": 1}`.l
    CONFIGS (list) : The rest of the available CD methods and their corresponding configurations 
    ...
    while CD_LIST!=[]:
        try:
            ...
            print(f"LOG : Causal structure : {cd_method.__name__} failed with error: {e}.")
            if verbose:
...
def _sim_prepare_data(
    ...
    if true_label is None:
        if cd_method=="PCMCI":    
            ...
            # adj_cp, adj_pd = safe_cd_task(true_data=true_data, cd_method="PCMCI", cd_kwargs=cd_kwargs, verbose=verbose)
            adj_cp, adj_pd = estimate_with_PCMCI(true_data=true_data, **cd_kwargs)
            # adj_cp.sum() > 0
        elif cd_method=="DYNO":
            if cd_kwargs is None:
                cd_kwargs = {
            ...
            # adj_cp, adj_pd = safe_cd_task(true_data=true_data, cd_method="DYNO", cd_kwargs=cd_kwargs, verbose=verbose)
            adj_cp, adj_pd = estimate_with_DYNOTEARS(true_data=true_data, **cd_kwargs)
        elif cd_method=="LCM":
            if cd_kwargs is None:
                cd_kwargs = {
            ...
            # adj_cp, adj_pd = safe_cd_task(true_data=true_data, cd_method="LCM", cd_kwargs=cd_kwargs, verbose=verbose)
            adj_cp, adj_pd = estimate_with_LCM(true_data=true_data, **cd_kwargs)
        elif cd_method=="CP":
            if cd_kwargs is None:
                cd_kwargs = {
                    "model": None,
            ...
            #     adj_cp, adj_pd = safe_cd_task(true_data=true_data, cd_method="DYNO", cd_kwargs=cd_kwargs, verbose=verbose)
                ...
                # adj_cp, adj_pd = safe_cd_task(true_data=true_data, cd_method="CP", cd_kwargs=cd_kwargs, verbose=verbose)
                adj_cp, adj_pd = estimate_with_CP(true_data=true_data, **cd_kwargs)
        ...
        # NOTE: used exclusively in the oracle graph experiment
            ...
            # adj_cp, adj_pd = safe_cd_task(true_data=true_data, cd_method="PCMCI", cd_kwargs=cd_kwargs, verbose=verbose) # subject to change
            adj_cp, adj_pd = estimate_with_PCMCI(true_data=true_data, **cd_kwargs)
    else:
        adj_pd = true_label
...
def _sim_fit_parameters(
    ...
    # Step 0: select forecaster & initialize placeholders
    scores = {}
    funcs_and_noise = {} 
    ...
    if pred_method=='RF':
        ...
        model = RandomForestRegressor(**pred_kwargs)
    elif pred_method=='KNR':
        if pred_kwargs is None:
            pred_kwargs = {'n_neighbors': 5}
        model = KNeighborsRegressor(**pred_kwargs)
    elif pred_method=='GBR':
        if pred_kwargs is None:
            pred_kwargs = {'n_estimators': 100}
        model = GradientBoostingRegressor(**pred_kwargs)
    elif pred_method=='TCDF':
        if pred_kwargs is None:
            pred_kwargs = {}
        model = TCDForecaster(**pred_kwargs)
    else:
