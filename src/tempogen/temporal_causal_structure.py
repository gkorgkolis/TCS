...
class TempCausalStructure:
    ...
    def __init__(
            ...
            seed=None,              # ...
            initial_graph=None,     # ...
            i_degree=2,             # specific to the (ID) method
            mode='norm'             # ...
    ) -> None:
        """
        Args
        ...
        i_degree (int) : the mean in-degree of each node; specific to the *ID* method
        mode (str) : concerns how the degrees for each node are sampled; specific to the *ID* method; possible values are 'norm' and 'comb'
        ...
        if causal_structure is None:
            if method=="C":
                ...
                causal_structure = self._in_degree_random_temporal_DAG(n_vars=n_vars, n_lags=n_lags, i_degree=i_degree, mode=mode, node_names=node_names)
    ...
    def _in_degree_random_temporal_DAG(self, n_vars=5, n_lags=2, i_degree=3, mode='norm', node_names=None):
        """
        ...
        *Note*: in contrast to the methods based on NetworkX functions, this method firstly creates 
        ...
        # print(f"LOG: Structure generation with in-degree {i_degree} and mode {mode}.")
        ...
        if mode=="norm":
            ...
            degrees = np.random.normal(loc=i_degree, scale=1, size=n_vars).round().astype("int")
        elif mode=="comb":
            # initialize the degree of each node by samping from all possible combinations that result in the desired mean degree
            possible_degrees = [list(x) for x in itertools.product(range(2*i_degree), repeat=n_vars) 
                                    if np.mean(x) in pd.Interval(left=i_degree-0.5, right=i_degree+0.5)]
    ...
    def plot_structure(self, temp_adj_pd=None, node_color='indianred', node_size = 800):
        ...
        temp_adj_pd = temp_adj_pd.loc[regular_order_pd(adj_pd=temp_adj_pd), regular_order_pd(adj_pd=temp_adj_pd)]
