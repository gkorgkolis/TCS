...
def plot_model_prediction(model, train_loader, len, save, name='test', path=None, mask=None, summary_writer=None):
    model.eval()
    targets = []
    preds = []
    ...
    plt.legend()
    plt.show()
    summary_writer.add_figure(name + '_prediction_single', plt.gcf())

    if save:
        if path is None:
            path = Path.cwd()
        path.mkdir(parents=True, exist_ok=True)
        plt.savefig(path / f"{name}_prediction_single.png")
    ...
    plt.plot(preds[0], label='pred')
    plt.legend()
    summary_writer.add_figure(name + '_prediction_batch', plt.gcf())

    if save:
        if path is None:
            path = Path.cwd()
        path.mkdir(parents=True, exist_ok=True)
        plt.savefig(path / f"{name}_prediction_batch.png")
    
    return targets, preds


def plot_generate_data(generated_data, ori_data, save_path, input_size, seq_length, summary_writer=None):
    ...
    #     summary_writer.add_figure('t-SNE Visualization', plt.gcf())
    ...
    # if summary_writer is not None:
    #     summary_writer.add_figure('PCA Visualization', plt.gcf())